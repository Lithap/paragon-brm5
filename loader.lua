...
local OPENWORLD_SRC = 
--------------------------------------------------------------------
--  PARAGON OPEN WORLD  •  Skeleton ESP (3 000 studs)
--------------------------------------------------------------------
if not game:IsLoaded() then game.Loaded:Wait() end
...
local targets={}
local w=setmetatable; local pool={
  hi = w({}, {__mode="k"}), trac=w({}, {__mode="k"}), lab=w({}, {__mode="k"}),
  hp = w({}, {__mode="k"}),  skel=w({}, {__mode="k"})
}
...
local function draw(tbl,id,k) if not DRAWING_OK then return end local o=tbl[id] if not o then o=Drawing.new(k) tbl[id]=o end return o end
...
local function LOS(p) local rp=RaycastParams.new() rp.FilterType=Enum.RaycastFilterType.Blacklist rp.FilterDescendantsInstances={LP.Character or Instance.new("Folder")} local hit=workspace:Raycast(Camera.CFrame.Position,p.Position-Camera.CFrame.Position,rp) return (not hit) or hit.Instance:IsDescendantOf(p.Parent) end

-- register enemies
local function enemy(m) if not(m:IsA("Model") and m.Name=="Male") then return false end for _,c in ipairs(m:GetChildren()) do if c.Name:sub(1,3)=="AI_" then return true end end return false end
...
workspace.DescendantRemoving:Connect(function(d) targets[d]=nil pool.skel[d]=nil end)

-- skeleton defs
local BONES={{"Head","UpperTorso"},{"UpperTorso","HumanoidRootPart"},{"HumanoidRootPart","LeftFoot"},{"HumanoidRootPart","RightFoot"},{"UpperTorso","LeftHand"},{"UpperTorso","RightHand"}}
local function skel(m) local a=pool.skel[m] if a then return a end a={} for _=1,#BONES do local l=Drawing.new("Line") l.Thickness=2 l.Visible=false a[#a+1]=l end pool.skel[m]=a return a end
local function hideSkel(m) local a=pool.skel[m] if a then for _,l in ipairs(a) do l.Visible=false end end end
...
RunService.RenderStepped:Connect(function(dt)
  ...
  for m,t in pairs(targets) do
    ...
    local r=t.root if not r then continue end
    local d=(r.Position-cam).Magnitude
    if d>MAX_DIST then hide(pool.trac,m) hide(pool.lab,m) hide(pool.hp ,m) hideSkel(m) if pool.hi[m] then pool.hi[m].Enabled=false end continue end
    local v2,on=Camera:WorldToViewportPoint(r.Position)
    local vis=(not OPT.vischeck) or LOS(r)

    -- skeleton
    if OPT.skeleton and on and DRAWING_OK then
      local ln=skel(m)
      for i,p in ipairs(BONES) do
        local a=m:FindFirstChild(p[1]) local b=m:FindFirstChild(p[2]) local l=ln[i]
        if a and b then
          local a2,onA=Camera:WorldToViewportPoint(a.Position) local b2,onB=Camera:WorldToViewportPoint(b.Position)
          if onA and onB then l.Visible=true l.From,l.To=Vector2.new(a2.X,a2.Y),Vector2.new(b2.X,b2.Y) l.Color=vis and Color3.fromRGB(255,0,0) or Color3.fromRGB(0,255,0) else l.Visible=false end
        else l.Visible=false end
      end
    else hideSkel(m) end

    -- chams
    if OPT.chams then local h=getHi(m) h.Enabled=true h.FillTransparency=0.15 h.OutlineTransparency=0.1 h.FillColor=vis and Color3.fromRGB(255,75,75) or Color3.fromRGB(0,190,255) h.OutlineColor=h.FillColor
    elseif pool.hi[m] then pool.hi[m].Enabled=false end

    if DRAWING_OK then
      -- tracer
      if OPT.tracers then local tr=draw(pool.trac,m,"Line") tr.Visible=true tr.Thickness=1.5 tr.Color=vis and Color3.fromRGB(255,0,0) or Color3.fromRGB(255,255,0) tr.From,tr.To=CENTER(),Vector2.new(v2.X,v2.Y) else hide(pool.trac,m) end
      -- distance
      if OPT.distance and on then local lb=draw(pool.lab,m,"Text") lb.Visible=true lb.Center=true lb.Outline=true lb.Size=14 lb.Color=Color3.new(1,1,1) lb.Text=("%.0f"):format(d) lb.Position=Vector2.new(v2.X,v2.Y-16) else hide(pool.lab,m) end
      -- health
      if OPT.health and on then local hum=m:FindFirstChildOfClass("Humanoid") if hum then local f=math.clamp(hum.Health/hum.MaxHealth,0,1) local hb=draw(pool.hp,m,"Square") hb.Visible=true hb.Filled=true hb.Size=BAR*Vector2.new(f,1) hb.Position=Vector2.new(v2.X-BAR.X/2,v2.Y+12) hb.Color=hpCol(f) end else hide(pool.hp,m) end
    end
  end
end)

-- clear
local function clr() for _,h in pairs(pool.hi) do h.Enabled=false end for _,t in ipairs{pool.trac,pool.lab,pool.hp} do for _,o in pairs(t) do o.Visible=false end end for m,_ in pairs(pool.skel) do hideSkel(m) end end

-- gui
local C_MAIN,C_ACC,C_TEXT=Color3.fromRGB(22,22,26),Color3.fromRGB(0,160,255),Color3.fromRGB(240,240,240) local ICON="✕"
(PG:FindFirstChild("ParagonMainUI"))?.:Destroy()
local g=Instance.new("ScreenGui",PG) g.Name,g.IgnoreGuiInset,g.ResetOnSpawn="ParagonMainUI",true,false if syn and syn.protect_gui then syn.protect_gui(g) end
local f=Instance.new("Frame",g) f.AnchorPoint=Vector2.new(0,0.5) f.Size=UDim2.new(0,270,0,340) f.Position=UDim2.new(0,-280,0.5,0) f.BackgroundColor3,f.BackgroundTransparency=C_MAIN,0.2 f.BorderSizePixel=0
Instance.new("UICorner",f).CornerRadius=UDim.new(0,8) Instance.new("UIStroke",f).Color=C_ACC
local h=Instance.new("TextLabel",f) h.Size=UDim2.new(1,0,0,40) h.BackgroundTransparency=1 h.Font=Enum.Font.GothamBlack h.Text="PARAGON ESP" h.TextScaled=true h.TextColor3=C_TEXT h.TextStrokeTransparency=0.85
...
local function tog(txt,k)
  local b=Instance.new("TextButton",bd) b.Size=UDim2.new(1,0,0,32) b.BackgroundColor3,b.BackgroundTransparency=C_MAIN,0.15 b.AutoButtonColor=false Instance.new("UICorner",b).CornerRadius=UDim.new(0,6)
  local t=Instance.new("TextLabel",b) t.BackgroundTransparency=1 t.Size,t.Position=UDim2.new(1,-28,1,0),UDim2.new(0,6,0,0) t.Font=Enum.Font.GothamSemibold t.TextScaled=true t.TextColor3=C_TEXT t.TextXAlignment=Enum.TextXAlignment.Left t.Text=txt
  local ico=Instance.new("TextLabel",b) ico.BackgroundTransparency=1 ico.Size,ico.Position=UDim2.new(0,22,0,22),UDim2.new(1,-26,0.5,-11) ico.Font=Enum.Font.GothamBold ico.TextScaled=true ico.Text=ICON
  local st=Instance.new("UIStroke",b) st.Color=C_ACC st.Transparency=0.8
  local function ref() local f=(k=="master" and ON) or OPT[k] ico.TextColor3=f and Color3.fromRGB(0,255,80) or Color3.fromRGB(180,180,180) end ref()
  ...
  b.MouseButton1Click:Connect(function() if k=="master" then ON=not ON if not ON then clr() end else OPT[k]=not OPT[k] end ref() end)
end
tog("ESP Master","master") tog("Skeleton","skeleton") tog("Chams","chams") tog("Tracers","tracers") tog("Distance","distance") tog("Health","health") tog("VisCheck","vischeck")
local open=false
local function slide() open=not open local y=-f.AbsoluteSize.Y/2 local tgt=open and UDim2.new(0,10,0.5,y) or UDim2.new(0,-f.AbsoluteSize.X-10,0.5,y) TweenService:Create(f,TweenInfo.new(0.45,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Position=tgt}):Play() end
UIS.InputBegan:Connect(function(i,gp) if not gp and i.KeyCode==Enum.KeyCode.BackSlash then slide() end end) slide()
--------------------------------------------------------------------
