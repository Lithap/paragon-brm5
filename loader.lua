--------------------------------------------------------------------
--  PARAGON • FINAL ONE-FILE BUILD  (no skeleton, no Valex spam)
--  key : paragon
--  July-2025
--------------------------------------------------------------------
...
--==============================================================--
-- 0️⃣  GLOBAL “Valex” SILENCER  (run before anything else)
--==============================================================--
local RunService = game:GetService("RunService")
local LogService = game:GetService("LogService")
local ScriptCtx  = game:GetService("ScriptContext")

-- hide console lines that still mention Valex
local function mute(msg) if msg:find("Valex") then return true end end
LogService.MessageOut:Connect(function(m,t)
    if t == Enum.MessageType.MessageError then return mute(m) end
end)
ScriptCtx.Error:Connect(function(m) return mute(m) end)

-- neutralise every script whose name or body contains 'Valex'
local function purge(inst)
    if inst:IsA("LocalScript") or inst:IsA("ModuleScript") then
        local ok,src = pcall(function() return inst.Source end)
        if inst.Name:lower():find("valex") or (ok and src and src:find("Valex")) then
...
for _,d in ipairs(game:GetDescendants()) do purge(d) end
game.DescendantAdded:Connect(purge)
RunService.Heartbeat:Connect(function() purge(game) end)   -- fast sweep

--==============================================================--
-- 1️⃣  KEY PANEL LOADER
--==============================================================--
...
local targets = {}
local w={__mode="k"}
local pool={ hi=setmetatable({},w), trac=setmetatable({},w), lab=setmetatable({},w), hp=setmetatable({},w) }
...
RunService.RenderStepped:Connect(function(dt)
  ...
  for m,t in pairs(targets) do
    ...
    -- chams
    if OPT.chams then local h=getHi(m) h.Enabled=true h.FillTransparency=0.15 h.OutlineTransparency=0.1
      h.FillColor=vis and Color3.fromRGB(255,75,75) or Color3.fromRGB(0,190,255) h.OutlineColor=h.FillColor
    ...
    if DRAWING_OK then
      ...
      if OPT.distance and on then
        local lb=draw(pool.lab,m,"Text") lb.Visible=true lb.Center=true lb.Outline=true lb.Size=14
        lb.Color=Color3.new(1,1,1) lb.Text=("%.0f"):format(dist) lb.Position=Vector2.new(v2.X,v2.Y-16)
      else hide(pool.lab,m) end

      -- health
      if OPT.health and on then
        local f=math.clamp(hum.Health/hum.MaxHealth,0,1)
        local hb=draw(pool.hp,m,"Square") hb.Visible=true hb.Filled=true
        hb.Size=BAR*Vector2.new(f,1)
        hb.Position=Vector2.new(v2.X-BAR.X/2,v2.Y+12)
        hb.Color=hpCol(f)
...
local f=Instance.new("Frame",g) f.AnchorPoint=Vector2.new(0,0.5) f.Size=UDim2.new(0,270,0,240) f.Position=UDim2.new(0,-280,0.5,0)
...
local function tog(txt,k)
  ...
  local t=Instance.new("TextLabel",b) t.BackgroundTransparency=1 t.Size,t.Position=UDim2.new(1,-28,1,0),UDim2.new(0,6,0,0)
  ...
  local function ref() local f=(k=="master" and ON) or OPT[k] ico.TextColor3=f and Color3.fromRGB(0,255,80) or Color3.fromRGB(180,180,180) end
  ref()
...
tog("Distance","distance") tog("Health","health") tog("VisCheck","vischeck")
local open=false
