...
RunService.RenderStepped:Connect(function(dt)
    ...
    for mdl,t in pairs(ESP.Targets) do
        ...
        elseif ESP.Cache.box[root] then ESP.Cache.box[root].Transparency=1 end
        ...
        elseif ESP.Cache.cham[mdl] then ESP.Cache.cham[mdl].Enabled=false end

        if DRAWING_OK then
            -- Tracer
            if ESP.Options.tracers and onScr then
                ...
                tr.From,tr.To = TRACER_ORG,Vector2.new(scr.X,scr.Y)
...
local labels={box="3-D Box",chams="Chams",tracers="Tracers",distance="Distance",health="Health Bar",vischeck="VisCheck"}
...
Rayfield:Notify({Title="Paragon BRM5",Content="Mini ESP v13 loaded – Right-Shift for UI",Duration=4})
